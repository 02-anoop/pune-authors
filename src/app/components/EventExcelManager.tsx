...
export default function EventExcelManager({
  ...
  return (
    <div className="flex flex-col gap-4 mt-8">
      <div className="flex justify-between items-center bg-[#00D8F5] p-3 border-2 border-black font-bold">
      ...
      <div className="overflow-x-auto border-2 border-black bg-white shadow-xl">
        <table className="w-full text-[11px] font-sans border-collapse whitespace-nowrap">
          <thead>
            <tr>
              <th rowSpan={2} className="border border-black bg-[#ffff00] p-1 w-10">S.No</th>
              <th rowSpan={2} className="border border-black bg-[#ffff00] p-1 w-48">Book Title</th>
              <th rowSpan={2} className="border border-black bg-[#ffff00] p-1 w-20">Amount<br/>Paid</th>
              <th rowSpan={2} className="border border-black bg-[#ffff00] p-1 w-16">MRP</th>
              <th rowSpan={2} className="border border-black bg-[#ffff00] p-1 w-32">Author Name</th>
              <th rowSpan={2} className="border border-black bg-[#ffff00] p-1 w-24">Suggested<br/>Number of<br/>Copies</th>
              <th rowSpan={2} className="border border-black bg-[#ffff00] p-1 w-24">Actual<br/>Number of<br/>Copies</th>
              {dayColumns.length > 0 && (
                <th colSpan={dayColumns.length} className="border border-black bg-[#ffff00] p-1 text-center">
                  Sales Record
                </th>
              )}
              <th rowSpan={2} className="border border-black bg-[#ffff00] p-1 w-20">Total<br/>Number of<br/>Books Sold</th>
              <th rowSpan={2} className="border border-black bg-gray-200 p-1 w-24">Actions</th>
            </tr>
            {dayColumns.length > 0 && (
              <tr>
                {dayColumns.map(dc => (
                  <th key={dc.label} className="border border-black bg-[#ffff00] p-1 w-16">{dc.label}</th>
                ))}
              </tr>
            )}
          </thead>
          <tbody>
            {authors.length === 0 ? (
              <tr>
                <td colSpan={10 + dayColumns.length} className="p-4 text-center text-gray-500 italic border border-black">
                ...
                if (!author.books || author.books.length === 0) {
                  return (
                    <tr key={author.authorId} className="hover:bg-gray-50 transition-all bg-gray-100/50">
                      <td className="border border-black text-black font-bold text-center p-1">{aIdx + 1}</td>
                      <td className="border border-black text-gray-400 italic p-1 px-2 text-center" colSpan={3}>
                        No books listed by author
                      </td>
                      <td className="border border-black bg-[#00ffff] text-black font-bold p-1 px-2 truncate max-w-[150px]">
                        {author.authorName}
                      </td>
                      <td className="border border-black text-gray-400 p-1" colSpan={3 + dayColumns.length}></td>
                      <td className="border border-black bg-white p-1 text-center">
                ...
                return author.books.map((book: any, bIdx: number) => {
                  ...
                  return (
                    <tr key={`${author.authorId}-${bIdx}`} className="hover:brightness-95 transition-all">
                      {isFirstBook && (
                        <td rowSpan={rowSpan} className="border border-black bg-red-600 text-white font-bold text-center p-1">
                          {aIdx + 1}
                        </td>
                      )}
                      
                      <td className="border border-black bg-[#ffcccc] text-black font-bold p-1 px-2 truncate max-w-[200px]" title={book.title || book.book?.title}>
                        {book.title || book.book?.title || "Unknown"}
                      </td>
                      
                      {isFirstBook && (
                        <td rowSpan={rowSpan} className="border border-black bg-green-400 text-black font-bold text-center p-1">
                          {author.amountPaid ? `₹${author.amountPaid}` : "NA"}
                        </td>
                      )}
                      
                      <td className="border border-black bg-[#ffddaa] text-black text-center p-1 font-mono font-bold">
                        {mrp}
                      </td>
                      
                      {isFirstBook && (
                        <td rowSpan={rowSpan} className="border border-black bg-[#00ffff] text-black font-bold p-1 px-2 truncate max-w-[150px]">
                          {author.authorName}
                        </td>
                      )}
                      
                      <td className="border border-black bg-[#ffddaa] text-black text-center p-1 font-bold">
                        {book.listedStock || 0}
                      </td>
                      
                      <td className={`border border-black text-black p-0 bg-white`}>
                        <input 
                          type="number"
                      ...
                      {dayColumns.map(dc => (
                        <td key={dc.label} className={`border border-black p-0 bg-white`}>
                          <input 
                            type="number"
                            className="w-full h-full p-1 bg-transparent border-none text-center outline-none font-bold"
                      ...
                      <td className="border border-black bg-white text-black text-center font-bold p-1">
                        {book.soldStock}
                      </td>
                      
                      {isFirstBook && (
                        <td rowSpan={rowSpan} className="border border-black bg-gray-50 p-1 text-center">
                          {isEditing ? (
      ...
      <div className="bg-gray-100 border border-gray-300 p-4 rounded-xl mt-2">
        <h4 className="font-bold text-sm mb-1 text-gray-700">Global Overrides (For events without individual author data)</h4>
        <p className="text-xs text-gray-500 mb-0">Use the main event dashboard to set global totals if individual books are not tracked.</p>
      </div>
    </div>
  );
}
